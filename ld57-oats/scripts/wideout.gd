...
@onready var speed_range: Vector2 = Vector2(50.0, 120.0)
...
func _on_shader_increment_timeout() -> void:
	...
	if not shader_updating:
		return
	var x_diff_from_start = position.x - starting_position.x
	if x_diff_from_start <= 0:
		return
	if blur_strength > 0:
		blur_strength -= (x_diff_from_start / 1500) # Hardcode for now
		print(blur_strength)
	shader_material.set_shader_parameter("blur_strength", blur_strength)
	if opacity_strength < 1:
		opacity_strength += (x_diff_from_start / 4500) # Hardcode for now
